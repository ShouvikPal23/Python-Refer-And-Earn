...
@JN.on_message(filters.command("add"),group=7)
async def handle_message(client, message):
    # print(message)
    if message.from_user.id==int(OWNER_ID):
    # if message.command[0].lower() == 'add':
        msg1 = await JN.ask(int(OWNER_ID), "How much money do you want to add? Please provide amount in INR format.")
        amount = float(msg1.text)
        print(amount)
        msg2 = await JN.ask(int(OWNER_ID), "Please provide the user ID to whom you want to add money.")
